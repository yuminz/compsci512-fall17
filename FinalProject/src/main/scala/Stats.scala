...
class Stats {
  ...
  var errors: Int = 0
  var sentMessageMap: mutable.Map[Long, Int] = new mutable.HashMap[Long, Int]
  ...
  def += (right: Stats): Stats = {
    ...
    errors += right.errors
    sentMessageMap = sentMessageMap ++ right.sentMessageMap.map{ case (k: Long, v: Int) => (k, sentMessageMap.get(k).getOrElse(0) + v)}
	  this
  }

  def printSentMessageMap(): String = {
    val sortedMap = sentMessageMap.toSeq.sortBy{ case (k: Long, v: Int) => k}
